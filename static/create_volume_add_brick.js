...
function replica_change_val() {
    ...
    if (brick_form.replica.value.valueOf() <= 0) {
...
window.onload = function () {
    ...
    brick_form.onsubmit = function () {
        if (brick_form.volume_name.value.length === 0) {
            alert("Volume name must not be empty.");
            return false;
        }
        if (brick_array.length === 0) {
        ...
        if (!(brick_form.transport_tcp.checked || brick_form.transport_rdma.checked)) {
            alert("Need at least one transport mode to be used.");
            return false;
        }
